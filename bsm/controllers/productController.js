...
const getAllProducts = async (req, res) => {
    try {
        const products = await Product.find().populate("categoryId").populate("companyId").populate("userId");
...
const getProductById = async (req, res) => {
    try {
        ...
        const product = await Product.findById(productId).populate("categoryId").populate("companyId").populate("userId");
        if (!product) {
    ...
    try {
      ...
      if (product.image) {
        ...
        if (publicId) {
          ...
          console.log("Could not extract publicId from image URL:", product.image);
        }
      }
...
const getProductByUser = async (req, res) => {
    try {
      ...
      const products = await Product.find({ userId }).populate("categoryId companyId", "name");
...
module.exports = {
    AddProduct,getAllProducts,getProductById,updateProduct,deleteProduct,getProductByUser
}