...
const AddProductStock = async (req, res) => {
  try {
    const { productId, purchaseBillId, totalStock, userId } = req.body;
    ...
    // console.log(Purchasebill, "purchase bill");
    if (!product) {
      return res
        .status(400)
    ...
    if (!Purchasebill) {
      return res
        .status(400)
        .json({ success: false, message: "Purchasebill not found" });
    }
    if (!totalStock) {
      return res
        .status(400)
        .json({ success: false, message: "totalstock not found" });
    }
...
const getProductStockByUser = async (req, res) => {
  try {
    ...
    const productstock = await ProductStock.findOne({ userId });
    // .populate(
    //   "productId"
    // );
    // .populate("purchaseBillId");
    console.log(productstock, "ps");
    return res.status(200).json({
      success: true,
      stock: productstock,
    });
...
const updateProductStock = async (req, res) => {
  try {
    ...
    if (!totalStock) {
      return res
        .status(400)
        .json({ success: false, message: "Total stock value is required" });
    }
    ...
    if (!productStock) {
      return res
        .status(404)
        .json({ success: false, message: "Product stock not found" });
    }
